...
static inline void enable_adc_timer()
{
    ...
    ///configure ADC registers
     ...
     R_S12ADC0->ADCSR_b.EXTRG = 0; //start A/D conversion with ELC (synchronous)

     //enable these input channels for u,v, and w bemf signals
     //the below register selects which channels are enabled, this register will change as the commutation patten changes
     ///these registers are written to when control switches from open loop to closed loop
//     R_S12ADC0->ADANSA0_b.ANSA00 = 1;
...
//     //enable sample and hold circuit
     ...
     R_S12ADC0->ADWINLLB_b.ADWINLLB = 0;
     R_S12ADC0->ADWINULB_b.ADWINULB = 0;
